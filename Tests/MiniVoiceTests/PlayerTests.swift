...
final class PlayerTests: XCTestCase {
    ...
    func testLibraryRescanAndTransport() async throws {
        ...
        let other = try XCTUnwrap(library.tracks.first { $0.id != first }?.id)
        library.seek(to: 4)
        library.select(other)
        XCTAssertEqual(library.selectedID, other)
        XCTAssertEqual(library.playingID, first)
        XCTAssertTrue(library.isPlaying)
        XCTAssertEqual(library.playbackTime, 4)
        library.pause()
        library.play()
        XCTAssertEqual(library.playingID, first)
        XCTAssertEqual(library.playbackTime, 4)
        library.play(other)
        XCTAssertEqual(library.playingID, other)
        XCTAssertEqual(library.playbackTime, 0)
        library.seek(to: 3)
        library.play(other)
        XCTAssertEqual(library.playbackTime, 0)
